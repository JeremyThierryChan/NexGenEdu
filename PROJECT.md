...
| 报课数量 | 试课、1 节（+10% 手续费）、5 节、10 节及以上（含试课） |
...
1. **科目系数**目前全部按 1.0，等实际系数确定后在 pricing.md 逐个补 `系数`。
2. **手续费语义**按「加价」实现（1 节 = 基础价 × 1.1）。若应为折扣需改 `quote.ts`。
3. 「10 节及以上」的合计按 10 节计算（数据文件里的 `节数: 10`），
   若按实际课时数计算，需要把节数改为用户输入。
