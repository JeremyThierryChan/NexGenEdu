 ...
 *
 * 判据就是"trim 之后非空"，只看 `campus` 那一格：服务层的写入闸、批量导入的行校验
 * 都调它，因此不可能出现"页面认为填了、导入认为没填"这种情况。
 *
 * 刻意**不**在这里顺手看名字能不能拆：要判的是**拆完之后**的 `campus`，
 * 而调用方本来就应该先把记录过一遍 `normalizeClassroom`（见 `normalizeClassroomStrict`
 * 与 `import.ts` 的行归一）—— 把"拆"这一步藏进这个判断里，等于同一件事有两个入口。
 */
...
/**
 * 「校区必填」这条口径的**唯一一处文案**（服务层与批量导入共用一句）。
 *
