 ...
 *
 * id 用卡片的路径（`course-site-<path>`）：**稳定且可重复**，因此重建库 / 灌夹具
 * 跑多少次都不会重复添加，也不会因为重新灌种子而变 id
 * （v32 起不再有运行期的「从网站同步」入口，但 id 的这条性质仍然被建库用到）。
 ...
 *
 * 这是"网站说名字、库里存 id"这个转换的**唯一实现**，两处共用：
 * 空库起步（`initial.ts`）与示例数据（`seed.ts`）—— v32 起那个运行期的「从网站同步」
 * 入口已删，但这两条建库路径仍然都要它。各写一遍的直接后果是"空库里多出一批重复分区"。
 *
...
export function materializeSiteCourses(
   ...
   * 空库起步与示例数据两条路都要这个结果，否则新建出来的课全是"没挂维度"，
...
/*
 * 这里以前还有一个 `canRemoveCourse()`：**"网站来源"的课不让删**。
 * 那条护栏已经删掉（2026-09，机构问「为什么现有的这些课程卡片不能删除」），
 * 因为它的前提被后面几轮改没了：
 *
 *   - v32 删掉了"从网站同步课程"那个入口 → **没有"删了下次同步又回来"这回事了**；
 *   - v39 / v40 把真源反过来了：**课程库是网站内容的真源**，`data/site/*.md` 是它的导出产物
 *     （`npm run site:export` 写回）。所以"卡片还在不在"是**结果**，不是"不许删"的理由；
 *   - 机构就是在正常地整理课程清单（先删了「高考冲刺」「特殊计划专项」），
 *     一条"网站来的就不许删"会让他们只能靠「暂未开放」绕，库里越堆越多死行。
 *
