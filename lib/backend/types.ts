...
export type Course = {
  ...
  name: string;
  /**
   * **这门课挂在哪些学段上**（v31 起，引用 `catalog.stages` 的 id）。
   *
   * 与下面两个字段一起，构成"课程 ←→ 课程类型"那座桥：台账按维度分组、
   * 组合开放能对上课程、将来的 AI 排课与诊断推荐按维度筛课程，都靠它们。
   * 全部按 **id** 引用（机构给学科/学段改名不影响挂在上面的课）。
   *
   * 空数组＝**还没挂**（迁移时按名字对不上的那些），台账里单独列一块提示手选 ——
   * 不猜：猜错的后果是排课与诊断按错的维度筛课，而页面上看起来一切正常。
   */
  stageIds: string[];
  /**
   * **这门课挂在哪些学科 / 项目上**（引用 `catalog.subjects` 的 id）。
   *
   * 为什么是**列表**：「高考外语」这种卡片是一张卡片覆盖五个语种（高考可以考日/俄/德/法/西），
   * 而维度表里没有"高考外语"这个学科。一门课挂多个学科因此是合理状态。
   */
  subjectIds: string[];
  /**
   * **这门课细到哪些内容模块**（引用 `catalog.modules` 的 id）；空数组＝不细分到模块。
   *
   * 大多数课程卡片本来就是"一个学科一张"，因此允许空；细分到模块的课（例如
   * 「小学语文 · 阅读」）在诊断推荐里才能说清"补的是哪一块"。
   */
  moduleIds: string[];
  /**
 ...
 * ## 为什么不是"一门课一条记录"
 *
 * 机构那份课程类型清单铺开有 400 多条叶子，而它们全是**四个维度**的乘积：
 * 学段 × 学科 / 项目 × 内容模块 × 班型。枚举法的代价在"加一个语种 / 加一级等级"
 * 时立刻显现：记录数翻倍、价格要逐条改、AI 排课与诊断推荐只能靠"一个诊断项硬绑几门课"。
 *
 * 因此这里只存**维度**（下面四张表的行），组合是**查询结果**：
 ...
 *
 * 曾经有过一版把「网课 / 网课+答疑 / 托管 / 全日托管」当成第五个维度"交付形态"
 * （"一门课可以面授，也可以网课+答疑"）—— 那是**错的**：机构的口径是
 * **「网课」「网课+答疑」「网课+一对一针对性答疑」「小学托管」…是独立的项目**，
 * 与小学 / 初中 / 高中那些按学段的课程**没有任何组合关系**。
 * 它们本来就在 `catalog.subjects` 里（`kind: "项目"`，挂在「不分班型项目」分组下），
 * 第五个维度只是把同一件事又说了一遍，还凭空给出了"小学语文 × 网课"这种不存在的组合。
 *
...
/**
 * 一条**开放的组合**（v24，`offers` 表）。
 *
 * ## 它解决什么
 *
 * 维度表回答"可以有哪些维度"，这一张回答"**本机构开哪些组合**"：
 * 一条组合 = 学科 × 内容模块 × 班型（例如「语文 · 客观题 · 一对一」）。
 *
 * 为什么**不把全部组合铺成行**：那是 133 个（学科 × 模块）格 × 5 班型
 * = **665 条**空行，而机构真正开的通常只是其中一小片。因此这张表是**稀疏**的：
 * 只有"机构表过态"的组合才有行 —— 于是有三种状态，而这三者是不同的：
 *
 * | 状态 | 数据 | 含义 |
 * | --- | --- | --- |
 * | 开放 | 有行，`open: true` | 明确开 |
 * | 明确关闭 | 有行，`open: false` | 机构**表过态**：不开（例如"这个班型不做网课"） |
 * | 没设置 | 没有行 | 还没人想过这件事 |
 *
 * 为什么保留"明确关闭"而不是删行：维度表改动后（新增一个班型、新增一个模块），
 * "没设置"的格子会一片片冒出来 —— 那才是需要机构去看的信号。把关闭也记下来，
 * 才能让矩阵里"已决策"与"待决策"一眼分开。
 *
 * ## 组合的**身份**是那三个 id
 *
 * `id` 由三个 id 拼出来（`offerId`，确定性、可重复执行），因此同一条组合不可能有两行、
 * 批量勾选也不会造出重复行。维度行改名不影响它（全系统按 id 引用，见 `Catalog` 的说明）。
 *
 * **价格不在这里**：组合价格仍是报价配置（`pricing`）里"课程 × 班型系数"那一套。
 * 把价放进这张表会让同一门课出现两个价源，而家长问价时对不上是最难查的一类错。
 */
export type CatalogOffer = {
  id: string;
  subjectId: string;
  /** 内容模块 id；**空串＝这条组合只到学科那一层**（不细分模块）。 */
  moduleId: string;
  formatId: string;
  open: boolean;
  /** 备注（给机构自己看，例如"只在寒暑假开"）。 */
 ...
 *
 * **v39 起界面上不再显示这个值** —— 机构口径：「教师标签里的『网站导入』能删掉吗？」
 * （v36 把「从网站导入教师」那个入口整块删掉之后，**以后不会再有新的档案是「网站」来源**，
 * 这个标只会出现在当年那几条老记录上，真实库里 5 位教师中 4 位是 `网站`，
 * 留着只是噪声，还容易和人事口径的 `Teacher.source`（招聘渠道）撞名）。
 *
 * **字段本身保留**：`origin` 记的是「**这条档案当初从哪来**」，那是一段历史事实 ——
 * 删字段要动数据库形状（迁移、`seed.ts`、`import.ts`、导出的教师映射、自检断言全在用），
 * 而"不假造历史"比"界面上少一个标"重要得多。因此它照旧由迁移与导入维护、
 * 导出时也照旧带出去，只是不渲染。
 */
...
export type Teacher = {
  ...
  kind: TeacherKind;
  /**
   * **用工性质**（v30 起）：全职 / 兼职，**空串＝未填**（见 `TEACHER_EMPLOYMENTS`）。
   *
   * 为什么"未填"要是独立的一档、而不是默认成「全职」：库里那几位网站的教师档案
   * 本来就是照着网站教师页建的，**机构从没登记过谁是全职谁是兼职**。
   * 猜一个值等于把"没填"记成一条假的用工事实，而人事口径上的错值是要拿去用的
   * （算排课量、算成本、对外说"我们有多少全职老师"）—— 因此照 v12/v13 补教师字段时
   * 那条纪律办：**不猜内容，一律空串**，由人一条条补。
   */
  employment: TeacherEmployment | "";
  /**
   * **招聘渠道**（v30 起，自由文本，可空）：这位老师是怎么来的 ——
   * 招聘网站 / 朋友介绍 / 内部推荐 / 校招 / 其他。
   *
   * ⚠️ **与 `origin` 不是一回事，别混**：
   *   - `origin` 回答「**这条档案从哪来**」（网站同步 / 后台手建）—— 技术来源，不可编辑，
   *     **界面上不再显示**（见 `Teacher.origin` 那里为什么删标但留字段）；
   *   - 这里的 `source` 回答「**这个人是机构从哪招来的**」—— 人事口径，要人手填。
   * 两者在中文里都能叫"来源"，因此界面上的标签刻意分开：只在人事口径那一处用「来源」；
   * 技术来源那边干脆不显示了。
   *
   * **内部信息**：不进宣传网站的公开数据（`public-site.ts` 的白名单里没有它），
   * 也不写进 `data/site/content.md` —— 招聘渠道是人事信息，公开仓库与 Pages 上都不该有。
   */
  source: string;
};
 ...
 * 教师的**用工性质**（v30 起）。
 *
 * `""`（未填）是**允许的第三档**，因此它不在这个常量里 —— 常量只列"人要选的两个值"，
 * 下拉与批量导入的候选值都由它生成（与 `CLASSROOM_KINDS` / `TEACHER_KINDS` 同一套写法）。
 * 校验的口径是「必须是这两个之一，或者空串」，那一条写在 `api.ts` 的
 * `normalizeTeacherRecord` / `teacherIssues` 里（**非法值报错拒绝，不是静默改成空串**：
 * 静默改会让"我明明填了兼职"变成一次无声的数据改动）。
 */
export const TEACHER_EMPLOYMENTS = ["全职", "兼职"] as const;
export type TeacherEmployment = (typeof TEACHER_EMPLOYMENTS)[number];
...
export type Classroom = {
  ...
  /**
   * **校区**（v30 起，自由文本；**v31 起写入时必填**）：这间房在哪个校区（「城西校区」「总校」
   * 这类机构自己的叫法）。
   *
   * 为什么是自由文本而不是一张校区表：机构现在只有几个场地，叫法也还没固定；
   * 建一张表就要先做"校区管理"那一页（增删改名排序），在字段本身还没用起来之前
   * 那一页没人维护。表单里给一个 `<datalist>` 列出**已在用的校区** ——
   * 既认得出旧写法、也不拦着新写法（与「教龄」那种自由文本同一条取舍）。
   * 等真的要按校区分权 / 分账 / 分统计时，再升级成一张表也不迟。
   *
   * **必填是"写入时"的（v31 收紧，机构原话「校区必须填」）**：类型上它仍是 `string`
   * （老库里"拆不出校区"的记录照样读得出来 —— 那种记录在类型上必须能表示，
   * 否则读取就得抛错，而"整库读不出来"比"有一间房待补校区"坏得多）。
   * 拦住它的是三道**写入时**的判据：服务层（`normalizeClassroomStrict`）、批量导入的行校验、
   * 以及教室表单的 `required`；判据是内容上的 —— **拆完「校区·教室名」之后校区非空**
   * （`hasCampus`）。界面上那种记录挂「校区未填」待补灰标。
   *
   * **内部信息**：不进宣传网站的公开数据（`public-site.ts` 的白名单里没有它）。
   */
  campus: string;
...
export type Database = {
   ...
   *
   * 稀疏存储（只有机构表过态的才有行），见上面 `CatalogOffer` 的说明。
   */
  offers: CatalogOffer[];
