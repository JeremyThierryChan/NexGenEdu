...
/**
 * 一个学习阶段（＝报价页上的一「组」）及其课程。
 *
 * ## `name` 是**网站栏目的名字**，不是学段（2026-09 口径，PROJECT.md 的 E15）
 *
 * = 每门课该组里的课 `partitionId` 指向的那个**顶级分区名**（`partitionPlace().column.name`）：
 * 小学课内 / 初中课内 / 高中课内 / 外语 / 课外兴趣 / 成人课程 —— 与课程页、课程总览
 * 用的是**同一套名字**。分组**完全由栏目决定**，一门课属于哪一组看它的栏目，不看别的。
 *
 * v37 那套「与课程类型的学段同名」（小学 / 初中 / 高中 / 其他类型）**已经作废**。
 * 学段仍然是**课程类型维度表**与**寒暑假段**的口径 —— 只是不再是报价的分组口径。
 *
 * ## ⚠️ 最容易踩回来的坑：**同名，但层级不同**
 *
 * 分区树里有**两个叫「外语」的分区**，而且它们不是一回事：
 *
 * | 分区 | 层级 | 里面是什么 |
 * | --- | --- | --- |
 * | `外语`（`cp_mudxlgqwaqm6`） | **顶级栏目** | 雅思、日语、俄语、法语、德语、意大利语、西班牙语 |
 * | `外语`（`cp_mudxlgqwp3go`） | **高中课内下面的子栏目** | 「高考外语」这门应试课 |
 *
 * 因此：**「高考外语」归「高中课内」**（它上面那一层栏目是高中课内），
 * **日语等语种课归「外语」**（顶级栏目）—— 两者**不在同一组**。
 * 机构为这件事专门提过一次：
 *
 * > 「**高中课内的 6 科高考外语和外语不能是一回事，虽然有同样的语言但是要单独分开**」
 *
 * **所以绝不能按"名字里带外语就放一起"来分组**，也绝不能拿 `name` 去比一门课
 * 所在分区的**叶子**名字：`partitionPlace()` 给出的 `column`（顶级）与 `leaf`（自己）
 * 在这里恰好同名，看着像"对上了"，其实是两个不同的分区。分组必须用 **`column`**。
 */
