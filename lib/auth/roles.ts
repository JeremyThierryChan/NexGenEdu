...
export const PAGE_ACCESS: Record<string, Role[]> = {
  ...
  /*
   * **课程**（后台一页三个页签：课程台账 / 课程类型 / 开放矩阵，v29 合并）。
   *
   * 路由权限取三个页面的**并集**：谁也不会因为合并少看到他本来能看的东西。
   * 页签各自的角色写在下面的 `ADMIN_COURSE_TABS`（与合并前那三条页面权限逐字一致），
   * 由页面按角色过滤 —— 因此"教师能进这一页"但"教师看不到课程类型与开放矩阵"。
   */
  "/admin/courses": ["技术管理员", "财务管理员", "招生老师", "普通教师"],
...
/**
 * **课程**那一页里三个页签各自的角色（v29 合并三个页面时新增）。
 *
 * 为什么单独一张表而不是复用 `PAGE_ACCESS`：合并之后只有一个路由，
 * 而这三块原本的角色并不一样 —— 台账给普通教师（只读），课程类型与开放矩阵不给。
 * 表里的角色值与合并前那三条页面权限**逐字一致**（自检有一条断言比对这件事，
 * 免得合并时顺手把谁放开或收紧了）。
 *
 * 顺序就是页签在页面上的顺序。
 */
export type AdminCourseTab = "ledger" | "dimensions" | "matrix";

export const ADMIN_COURSE_TABS: Array<{
  key: AdminCourseTab;
  /** 地址里的锚点（`#dimensions` 这种可以直接分享）。刻意用 ASCII：中文锚点会变成一长串百分号编码。 */
  hash: string;
  label: string;
  /** 页面标题下那句话（跟着当前页签变）。 */
  blurb: string;
  /** 页签旁边那句提醒（三块的保存语义不同，见页面文件头）。 */
  hint: string;
  roles: Role[];
}> = [
  {
    ...
    hash: "ledger",
    label: "课程台账",
    blurb: "实际开的课：分区、卡片、网站正文与报价。排课的科目、教师可带科目、报课科目都取自这里。",
    hint: "这门一块是**逐条保存**（改哪门课保存哪门课，带乐观锁）。",
    roles: ["技术管理员", "招生老师", "普通教师"],
  },
  {
    key: "dimensions",
    hash: "dimensions",
    label: "课程类型",
    blurb: "坐标系：学段 × 学科 / 项目 × 内容模块 × 班型。课程由这几个维度组合而来，不逐门枚举。",
    hint: "这一块是**整份保存**（四张维度表一起交，服务端逐条校验）。",
    roles: ["技术管理员", "财务管理员", "招生老师"],
  },
  {
    key: "matrix",
    hash: "matrix",
    label: "开放矩阵",
    blurb: "组合：哪些「学科 × 内容模块 × 班型」真的开放。供 AI 排课与诊断推荐判定。",
    hint: "这一块也是**整份保存**（勾完一次交）。",
    roles: ["技术管理员", "财务管理员", "招生老师"],
  },
];
