 ...
 *
 * ⚠️ **2026-09 分组口径又变了一次（见 PROJECT.md E15）**：分组**不再按学段，改成按「网站栏目」**
...
eq("外语的课程与价格（独立语种课 + 雅思，与「高考外语」分开）",
  ...
  ["雅思=700", "日语=暂未开放", "俄语=暂未开放", "法语=300", "德语=300",
    "意大利语=暂未开放", "西班牙语=300"]);
// 机构口径：「各种类型的课外兴趣都得是一个价格，150/小时」（原来两门都是暂未开放）
eq("课外兴趣的课程与价格（整组同价 150 元/小时）",
  pricing.stages[4]?.courses.map(priceLabel),
  ["3D建模 & 3D打印=150", "编程与信息素养=150"]);
ok("课外兴趣这一组里的每一门课都是同一个价（机构口径：这一栏目统一 150）",
  (pricing.stages[4]?.courses ?? []).every((course) => course.price === 150));
eq("成人课程的课程与价格",
...
/*
 * 「**每一组都有可报价的课**」—— 这条判据回来了，而且这次是被机构的数据**验证过**的。
 *
 * 它中间红过一次（2026-09 分组改成按栏目之后）：那时「课外兴趣」两门课都是「暂未开放」，
 * 整组没有一门可报价。当时的做法是**如实钉住那个状态 + 指向机构待办**，而不是把断言放宽
 * （放宽了，将来真出问题也没人会知道）。机构随后给了口径：
 * 「**各种类型的课外兴趣都得是一个价格，150/小时**」→ 两门课都开放并定价 150（元/小时），
 * 这一组现在有东西可报价了，因此这条判据恢复成不变式。
 */
ok("每一组都有可报价的课（否则家长在报价页点进这一组是空的）",
 ...
 *      —— **2026-09 起分组口径从"学段"改成"网站栏目"**（见 §4 与 PROJECT.md E15），
