...
| 维度 | 含义 | 默认 |
| --- | --- | --- |
| 基础价 | **每小时的价（元 / 小时）**：一对一、报 2 节及以上；课单价（元 / 节）= 它 × 每节课几小时 × 科目系数 × 班级系数 | 分阶段设定 |
...
- `subjects[]` 的 `stageName` 必须指向存在的阶段，`name` 是该学段里真有的学科
  （「其他类型」那一组不分科目，因此没有它的行）。
  自检第 41 节按这三条断"报价与课程清单一一对应"。
...
- 站点侧取哪一份，看**构站时的来源**（`siteContentSource()` 三态）：
  ...
  `blank`（没连上后端）→ 阶段 / 科目 / 班级系数**为空**，只有页面文案来自模版；
