...
| `Classroom` | `campus` | `string`（自由文本，可空） | 「校区」 | 机构自己的叫法（沐阳教育 / 全慧教育…），表单里给 `<datalist>` 列出已在用的校区。**与「教室名称」分开输入、显示时拼成「校区·教室名」**（v31，见 6.7.1） |
...
两列都写了、或名称列里还写着老的合并写法，都会被同一套规则归一 ——
...
不再引导用户填带「·」的完整写法）与「校区」（`<datalist>` 列出已在用的校区）。
