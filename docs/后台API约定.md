...
- 数据类型带 `version`（当前 **v30**），升级链在 `api.ts` 的 `migrate()`，
...
`courses.remove` 只拦**被引用着**的课（`courseDeleteRefusal`：有报课或排课引用着这个科目名
→ 拒绝删除并说清先做什么）。**网站来源的课同样能删**（2026-09 起：v32 删了同步入口、
v39/v40 又把真源反成"课程库 → 内容文件"，因此"网站来的"不再是"不许删"的理由）；
删掉之后报价配置里那一行也会跟着删掉（见下面「报价配置」那一节）。

**课程分区**（v18）：课程挂在**分区**上（`Course.partitionId`），分区是单独的记录集合，
名字与层级只写在它一处 —— 于是改名是"一处改、处处变"，而不是逐门课去改分类字符串。
服务端必须复核四条：分区名非空且**同级不重名**、上级存在、**最多两级**（栏目 → 子栏目）、
不能挂到自己（或自己的下级）下面形成环；删除时**有课或有子栏目一律拒绝**并说明先做什么
（判据是 `partitionDeleteRefusal`，与学员/课程的删除护栏同一套做法）。
`courses.setPartition(ids, partitionId)` 一次把一批课移过去：一个事务、一条日志，
中途不会留下"移了一半"的状态。
...
| 维度 | 含义 | 默认 |
| --- | --- | --- |
| 基础价 | **每小时的价（元 / 小时）**：一对一、报 2 节及以上；课单价（元 / 节）= 它 × 每节课几小时 × 人数系数 | 分阶段设定 |
| 人数系数 | 人越多每人越便宜（讲的是**人数**，不是"班级"） | 一对二 0.7、一对三 0.6、一对多 0.5 |
| 时长乘数 | 一节课上多久 | 1 / 1.5 / 2 小时 → 1.0 / 1.5 / 2.0 |
| 手续费 | 只报 1 节时加收 | 10% |
| 试课 | 报满多少节后试课免费，否则按原价收 1 节 | 10 节 |

价格维度是**三个**（基础价 / 人数系数 / 时长乘数），外加手续费与试课两条规则。
「科目系数」那一维已经**整维删除**（机构原话：「科目系数可以删除，班级系数改成人数系数」）：
**「班级系数」改名为「人数系数」**（含义与数值一个字没变，它管的是人数），
科目这一维连同 `PricingConfig.subjects` 一起没有了 —— 为什么删掉不改任何已算出的价，见 PROJECT.md 的 E10。

两个例外：**班课**按「教师课时总费用 ÷ 班级人数」分摊（不用人数系数）；
**试课费**永远按课程基础价原价收（不带任何系数与手续费）。

**「阶段」与「课程名」的口径（v37 以课程清单为准；2026-09 分组再改成「网站栏目」）**：

- `stages[].name` = **网站栏目的名字**（＝每门课 `partitionId` 指向的**顶级分区名**：
  小学课内 / 初中课内 / 高中课内 / 外语 / 课外兴趣 / 成人课程），与课程页、课程总览
  用的是同一套名字；没有课的栏目不建组。
  ⚠️ **v37 时这里写的是"课程类型里的学段名"，那条口径已经作废**（机构原话：
  「**高中课内的 6 科高考外语和外语不能是一回事，虽然有同样的语言但是要单独分开**」——
  按学段分组会把「高考外语」（栏目＝高中课内）与日语等语种课（栏目＝外语）放进同一组，
  见 PROJECT.md 的 E16）。学段仍然是**课程类型维度表**与**寒暑假段**的口径，
  只是不再是报价的分组口径。
- `stages[].courses[].name` = **课程库里的课程名**，且整份配置里**不许重名**
  （`validatePricingConfig` 拦；报价按名字查课，重名会报错价）；
  `courseId` 是它与课程库的关联，改课名 / 停开课程 / **删课**都由 `syncPricingWithCourses` 跟着改。
  自检第 4 节与第 41 节按这两条断"报价与课程清单一一对应"，其中第 41 节比的是
  **每门课 `partitionId` 的顶级分区名**（不是 `stageIds`）：报价里的每个分组都对应
  一个真实的顶级栏目，每门课也都待在自己栏目那一组里。
- **课程库里没有的课，报价里不许留行**（2026-09 口径，PROJECT.md 的 E14）：
  - 机构在台账里删掉一门课 → 报价配置里**那一行也删掉**（连同空掉的阶段），
    日志写「「X」在课程库里已不存在 → 报价配置里已删除这一行」；
  - 老库 / 导入进来的库里如果还留着这种残行，迁移链末尾的**收尾归一**会清掉它们，
    并写一条「报价 清理」日志（点名清了哪几行、哪一组空了）；
  - 判据只有一处：`prunePricingOrphans()` —— 「这一行**挂过** `courseId`，而库里没有这个 id 了」。
    从没挂过课程 id 的行（内容文件里手写的老课包名）**不是**残行，原样留着；
  - **当年那条「不静默删除」（保留名字、只置成暂未开放）已经作废** —— 它的后果就是
    机构看到的"删掉的课还是会出现"。为什么现在可以安全删：真正还在被用的课**本来就删不掉**
    （`courses.remove` 有 `courseDeleteRefusal`：被报课 / 排课引用着就拒绝删除）。

这一组接服务端时要注意的地方（现在这些校验都在服务端跑，因此也成了"别哪天搬回前端"的约束）：

- `pricing.quote` 只接受「课程名 / 班型 / 时长 / 节数」，**不接受价格**；
- `pricing.update` 必须自己复核配置合法性（系数为正、手续费 0–100、课程名不重复），
  否则系数写 0 会让所有报价变 0、写错类型会变成 ¥NaN，而这些会直接显示给家长；
- 改价要留审计（谁在什么时候把哪门课从多少改到多少）；
- 站点侧取哪一份，看**构站时的来源**（`siteContentSource()` 三态）：
  `backend`（本机构建、后端在跑）→ 报价整块来自**后端快照**（`sync-site-data` 取的那一份）；
  `blank`（没连上后端）→ 阶段 / 人数系数**为空**，只有页面文案来自模版；
  `template`（显式 `SITE_CONTENT_SOURCE=template`，线上 CI 用的是这个）→ 用
  `data/site/pricing.md` 那一份。
  因此 `pricing.exportMarkdown` 导出与 `data/site/pricing.md` 同构的片段，
  **是给 template 那条路用的**（替换进内容文件并提交才上线）；本机构建那条路重新构站即可 ——
...
**教师分成（课时费）**：课内课程里按系数计价的班型（一对一 / 一对二 /
一对三 / 小班课（4-8人），1–8 人）适用下面这条规则，大班课（9-20人）不适用
（那类按「教师课时总费用 ÷ 班级人数」另议）：
...
公式里的「课程单价」默认取**课程标准单价**（就是**基础价**本身，不含人数折扣），
也可切换为**班型课时价**（**基础价 × 人数系数** = 家长每生实付）—— 两种口径后台一键切换。
...
报课记录建好之后要能改：换了老师、班型从一对一转成小组课，都是常事。
...
#### 400 与 500：「你填错了」和「我们写错了」不许混（按异常类型分，不猜文案）

`/api/call` 只按**异常的类型**翻状态码，因此还有一条同类的分界 —— 服务层里两种 `throw`
的**读者不同**：

| 服务层抛的是什么 | 状态码 | 响应体 | 谁该看 |
| --- | --- | --- | --- |
| `throw new Error("课时不足：还能排 3 节。")`（业务拒绝，全项目都是这个写法） | **400** | **原文照回**（就是这句话） | 操作人 —— 改一改就能成 |
| `TypeError` / `RangeError` / `ReferenceError`，以及非 `Error` 的值 | **500** | 只有一句人话 + 一个 `req_xxxxxx` 编号 | 开发 —— 参数怎么改都没用 |

500 的响应体长这样，**不含**内部细节（字段名、文件路径、堆栈都不出现）：

```
服务端处理这次请求时出错了（编号 req_tl9s5h）。请刷新这一页重试；仍然不行就把这个编号告诉开发
```

完整原因与堆栈同时打到**服务端日志**（`[请求失败 req_tl9s5h] POST /api/call TypeError: …`），
排障用编号对上。为什么必须分开：早先一律回 400 加原文，出过一个真实后果 ——
`pricing.quote` 的代码 bug（读了个 `undefined` 的字段）在界面上被当成「你填错了」弹出来，
排障方向被带偏半天；而且那句 `Cannot read properties of undefined…` 是回给**局域网里任何
设备**的。这条口径由 `scripts/check-auth.mts` 的 `[13]` 节拿真实 HTTP 钉住
（业务拒绝必须 400 且不带 `req_`；代码 bug 必须 500、带编号、且不出现 `TypeError` 字样）。
...
### 6.7 `Teacher` / `Classroom` 的内部字段（v30：校区 / 全职兼职 / 来源；v31：教室名的拆分与显示）

机构原话：「**在教室页面里添加一个校区字段吧，教师界面也添加一个全职/兼职以及教师来源**」。
三个字段都是**内部信息**（详见下面最后一段），加在 `lib/backend/types.ts`：

| 实体 | 字段 | 类型与取值 | 界面标签 | 说明 |
| --- | --- | --- | --- | --- |
| `Teacher` | `employment` | `"全职"` / `"兼职"` / `""`（**空串＝未填**） | 「全职 / 兼职」（下拉，含「未填」一项） | 候选值只有一份定义：`TEACHER_EMPLOYMENTS`。**非法值服务端报错拒绝**。列表上未填时显示**灰色待办小标**「用工未填」（不是隐藏，见下面那段） |
| `Teacher` | `source` | `string`（自由文本，可空） | 「来源」 | **招聘渠道**（招聘网站 / 朋友介绍 / 内部推荐 / 校招 / 其他），表单里给 `<datalist>` 列出已在用的值。列表上未填时显示灰色「来源未填」 |
| `Classroom` | `campus` | `string`（自由文本，**必填** —— v31 收紧；服务层 / 导入 / 表单三处都判） | 「校区」 | 机构自己的叫法（沐阳教育 / 全慧教育…），表单里给 `<datalist>` 列出已在用的校区。**与「教室名称」分开输入、显示时拼成「校区·教室名」**（v31，见 6.7.1）。**空串 / 只有空格 → 报错拒绝**（判据是「拆完之后」的校区，见 6.7.2） |

**未填也要看得出来**（机构口径：「**教师信息我自己在后台填**，更希望**没填的时候也看得出来**
（比如卡片上显示一个灰色的「用工未填」，提醒你去补）」）。因此教师列表上这两个人事字段是
**两态**的，而不是"有值才显示"：

| 状态 | 卡片上 | 样式 |
| --- | --- | --- |
| 填了 | `全职` / `兼职`、`来源 朋友介绍` | 白底描边小标 / 普通弱化文字（已填那一档） |
| **没填** | **`用工未填`** / **`来源未填`**（`title="点姓名展开就能填"`） | **虚线边框 + 灰底 + 更浅的字色**（待办那一档） |

两态样式**必须一眼分得开**：同一档的话，那个灰标会被读成"这个人的用工性质叫『用工未填』" ——
而机构正是拿这两个字段算排课量与成本的，一条**假的人事信息**比不显示更糟。
卡片上那一排小标的顺序是 **AI → 教龄 → 用工性质 → 来源**：缺一条信息不该抢在身份与资历前面。

这两个字段**只由人自己填**：迁移与示例数据之外的教师记录，系统**一个值都不写**
（v29 → v30 那几个内部字段全部补空串、v30 → v31 只碰教室表）。自检第 44 节有一组源码级断言盯着：
"空值那一支渲染的是「用工未填」而不是 `null`／不渲染"、两态样式分得开、灰标带提示、
以及小标的顺序。

**校验与归一**（`teachers.create` / `teachers.update` / `classrooms.create` / `classrooms.update`）：

- 归一（`normalizeTeacherRecord` / `normalizeClassroom`）挂在**唯一的写入闸**上
  （`versionedCollection` 的 `normalize` 钩子）：`create` 与 `update` 都过一遍，
  `source` / `campus` 去前后空白，教室还要按 §6.7.1 拆一次「校区·教室名」；
- **`employment` 只认 `全职` / `兼职` / 空串**，其它值（`临时工`、`true`、数字…）**报错拒绝**，
  错误里带上原值与允许的取值。**不是静默压成空串** —— 那会让"我填的是兼职"变成一次
  没有痕迹的数据改动（操作日志里只写着"改了几列"）；
- **`campus` 必填**（v31 收紧，见 §6.7.2）：空串 / 只有空格 → **报错拒绝**；
- **字段没给 ≠ 给错了**：入参里根本没有 `employment` 这个键时按"未提供"处理（补空串＝未填，
  与迁移同口径），这样不知道新字段的老调用方与脚本不会被一棒打死；
- 拒绝的那一次**什么都不写**：钩子抛在 `persist` 之前，连记录版本号都不推进；
- ⚠️ 教师那个钩子是**先校验后归一**、教室那个是**先拆后判**（顺序相反，各有理由，见 §6.7.2）。

**迁移（v29 → v30）**：老库一律补**空串**，**不猜内容**（与 v12 → v13 给教师补资料那一步同一条纪律）。
这一步尤其不能猜：库里那几位教师是照网站教师页建的，机构从没登记过谁是全职谁是兼职 ——
默认成「全职」等于凭空记下一条会被拿去算排课量、算成本的**人事事实**；教室名里虽然写着
「沐阳教育·教室1」这种看得出校区的叫法，但"看得出"不等于"机构登过记"。
`migrate()` 的**收尾归一**还会兜一次这三个字段（一份"自称 v30 却缺字段"的导入 ——
手改过的导出、只跑了一半的恢复 —— 会在那一层补上空串，而不是让教师页的下拉读到 `undefined`）。

#### 6.7.1 `Classroom` 的两个字段怎么合成一个名字（v30 → v31）

机构在加了「校区」之后补了一条口径（原话）：

> 「**沐阳教育·教室1**，现在这个显示格式就是校区·教室名，现在添加了校区字段，
> 也就意味着我需要这个**卡片显示格式不变**，但**输入的时候校区和教室名称要单独输入**」

于是 v31 做了三件事，**显示格式一个字没变**：

| | 做什么 | 在哪 |
| --- | --- | --- |
| **显示** | `campus` 非空 → `` `${campus}·${name}` ``（中文间隔号、**不加空格**）；空 → 纯教室名。全后台给用户看教室名的地方（排课 / 批量排课 / 课表 / 日历 / 今日概览 / 教室页卡片 / 统计 / 冲突与拒绝文案 / 搜索 / ICS 的「地点」）都走 `classroomLabel(room)` | `lib/backend/classrooms.ts`（唯一一处；自检第 45 节有源码级断言，允许的例外逐条写在断言旁边） |
| **拆分** | `campus` 为空且名字含「·」→ 按**第一个**「·」拆成两格；`campus` 有值而名字里又粘着同一段前缀 → 去掉那段重复前缀；其余原样（**幂等**：`campus` 已非空就不再拆） | `normalizeClassroom` / `splitCampusFields`（同上，**一处实现**） |
| **迁移** | v30 → v31：老库那 6 间「沐阳教育·教室1」这类记录拆成 `campus` + `name`（真实库 → 沐阳教育 / 全慧教育 + 教室1 / 教室2 / 教室3 / 自习室 / 落地房四楼小 / 落地房四楼大），拆了几条写进操作日志 | `api.ts` 的 `migrate()` |

**拆分不改变显示**（拆完 `classroomLabel` 拼回来与原文一字不差），因此它不是"静默改数据"，
而是一次口径归一 —— 这也是它敢放在**读时归一**里的原因。归一有**四个调用点**，
共用同一个实现：v31 迁移、`migrate()` 的收尾归一、批量导入、服务层写入。

**批量导入**（教室实体）：「名称」只填**房间本身**的名字，「校区」单独一列（v30 起就有、
**v31 起必填**）。两列都写了、或名称列里还写着老的合并写法，都会被同一套规则归一 ——
关键是**判重发生在拆分之后**（判重键是 `name`），因此把一份旧名单再导一次**不会多出一间重复的房**。
教室那张导出的 CSV 与这个同形（「校区」「名称」两列，名称**不拼**校区），导出/导入可往返。

**表单**：教室表单里是**两个独立输入框** ——「教室名称」（placeholder 例如 `教室1 / 落地房四楼小`，
不再引导用户填带「·」的完整写法）与「校区」（`<datalist>` 列出已在用的校区，**`required`**）。
列表与卡片上仍按 `classroomLabel` 显示，因此看到的还是「校区·教室名」。

#### 6.7.2 `Classroom.campus` 为什么必填、以及在哪儿判（v31 收紧）

机构回答了那个"要不要加个开关"的问题（原话）：**「校区必须填」**。

背景：上面那条"校区空 + 名字含「·」就拆"的规则有一个**已知含义** ——
名字里真带「·」而校区空着的房间会被当成合并写法拆开（显示不变，但校区会多一个值）。
与其加一个开关让人去选，机构选的是**让那个状态不再出现**。

**判据落在「拆完之后」**（`hasCampus`，只判 `campus` 那一格 trim 之后是否非空）：
...
最后一行是刻意留的口子：它与**批量导入**是同一条口径（那里的行归一也是拆在前、判在后）。
判在拆分之前的话，一份"名称列里写着「沐阳教育·教室1」"的旧名单 / 一个只知道合并写法的旧调用方
会被**整份拒掉**，而它们本来是能建、能导的 —— 收紧要收的是"校区空着"这个状态，不是废掉旧写法。
（**界面表单走不到那一行**：校区那一格已经必填，空着就提交不了。）

三条路**都要判**，少一处就会出现"换个入口就能建出一间没校区的房"：

| 在哪 | 实现 | 错误原话 |
| --- | --- | --- |
| 服务层 `create` / `update` | `normalizeClassroomStrict`（`versionedCollection` 的 `normalize` 钩子，**先拆后判**） | `校区必填 —— 它决定教室在列表与排课里显示成「校区·教室名」，也用来按校区筛。` |
| 批量导入 | `import.ts` 的 `FieldSpec.derivableFrom: "name"`（**不进"整份不导入"的表头闸**，改成行归一之后逐行判） | `缺少必填列：校区（这一行没填校区，名称里也没有「校区·教室名」的写法可拆开）` |
| 教室表单 | `required` + 提交前那句 JS 校验 | 同服务层那一句（前端校验只是"早点告诉人"，不是唯一一道门） |

**读取与迁移不判**：老库里可能真的躺着"名字里没有「·」"的教室（例如「自习区」），拆不出校区。
在那里抛错等于**整库读不出来**（迁移的纪律是"不猜、不拒"，只补空串）。那种记录照常读出来、
照常显示、照常排课，教室页卡片上挂一个**「校区未填」待补灰标**（与教师卡片上「用工未填」同一档样式），
**保存的时候**才被必填拦住 —— 也就是要改它的任何字段，得先把校区补上。

**`source` 与 `origin` 是两个东西，别混**：

| | `Teacher.origin`（已有，v13） | `Teacher.source`（v30 新增） |
| --- | --- | --- |
| 回答什么 | **这条档案从哪来**（技术来源） | **这个人是机构从哪招来的**（人事口径） |
| 取值 | `网站` / `后台` | 自由文本（招聘网站 / 朋友介绍 / …） |
| 谁能改 | 不可编辑，由导入 / 建档那一侧决定 | 人在教师表单里填 |
| 界面上 | **不显示**（v39 起：那个小标删掉了，见下面） | 表单里的「来源」输入框 + 列表里的「来源 …」 |

**`origin` 那个小标已经删掉**（v39，机构口径：「教师标签里的『网站导入』能删掉吗？」）。
为什么可以删：v36 把那个"从网站拉一份教师档案"的入口整块删掉之后，**以后不会再有新的档案是
「网站」来源** —— 这个标只会出现在当年那几条老记录上（真实库里 5 位教师有 4 位是 `网站`），
留着只是噪声，而且容易与人事口径的「来源（招聘渠道）」撞名。

**字段本身保留，一个字没动**：`origin` 记的是「**这条档案当初从哪来**」，那是一段历史事实 ——
删字段要动数据库形状（迁移、`seed.ts`、`import.ts`、导出的教师映射、自检断言全都在用它），
而"不假造历史"比"界面上少一个标"重要得多。它照旧由迁移与导入维护、导出时也照旧带出去，
只是**不渲染**。自检第 44 节两条断言一正一反钉住这件事：卡片上不再出现 `origin`／「网站导入」，
而字段仍在类型、迁移与导入里。

**批量导入**（`imports.apply` 的 `ENTITY_SPECS`）：教师多两列「**全职兼职**」（`aliases` 也认
「全职/兼职」「全职 / 兼职」「用工性质」）与「**来源**」，教室多一列「**校区**」。
「全职兼职」是 enum：非法值**在解析这一层就被拦下**（指出第几行、哪一列），
而且**空单元格导出「未填」而不是第一个候选值** —— 列上多了一个 `empty: ""` 声明
（默认行为是取第一个候选值，那是给「类型」那种"留空＝没说"的列用的；
用工性质留空是真的"未填"这一档，不能替人填成「全职」）。
`csvTemplate` / `jsonTemplate` 跟着列定义一起变，不必单独改。

**不许泄漏到公网**（硬要求）：这三个字段**不**在 `lib/backend/public-site.ts` 的字段白名单里
（那是对外公开数据的唯一构造处，`GET /api/public/site` 是匿名可用的），
也**不**写进 `data/site/content.md` —— 招聘渠道属于人事信息，公开仓库与 Pages 上都不该有。
`scripts/check.mts` 第 44 节用四条断言钉住：公开快照的**键名**里没有它们、快照**正文**里没有
「全职 / 朋友介绍」这类值、白名单源码里没有它们（去掉注释再查）、
导出成 `data/site/*.md` 的内容里也没有（同样是键 + 值两道）。
...
2. ✅ **实现接口**：没有逐个接口重写，而是让服务端**复用整份 `api.ts`**：
   ...
   与一个通用分发入口 `/api/call`。
   **参考 REST 接口（`/api/students` 等）已于 2026-09 整条下线**（现在一律回 **410 Gone** 并指路
   `/api/call`）：它们读写的是另一套规范化表，与界面用的快照不是同一份存储，
   于是会给出"写进去没人读"的假成功与"读到空数据"的假失败；而**删除护栏原先只挂在它们身上**
   （审计的第一条：界面走的那条路完全绕开了护栏）。实现与权限表都已删干净，
   留着 `LEGACY_REST_PATH` 只是为了给这些路径回一句清楚的 410 与指路，而不是"悄悄 404"。
