...
- 内容在**后台**（教师页 / 课程台账 / 「网站内容」页 / 报价）维护；`data/site/*.md` 这六个文件是它的**导出产物** ——
  改完后台跑 `npm run site:export`（写回六个 `.md` 并自动接着跑 `sync-content`），本地 dev 会跟着重编译（无需重启 dev server）；
- 智能报价：学习阶段 / 课程 / 科目 / 班型 / 时长 / 节数，算出课单价、总价与试课费。
  分组就是**课程类型里的学段**，课程名与课程库**同一套**（v37：以课程清单为准；改课程名或停开课程，报价自动跟随）。
...
- 今日概览、咨询（排课可行性）、学生档案（含信息采集表）、教师、教室、课程安排、日历、**节假日**、课表与占用、统计、待跟进、收费、报价、课程库（含**课程分区**：栏目 → 子栏目可增删改名排序）、**网站内容**（学生案例 · 特色课程）、数据与备份、**账号**；
...
- **批量导入**：学生 / 教师 / 教室 / 课程可用 CSV 或 JSON 成批导入（各实体页右上角「批量导入」，或「数据与备份」页的面板）；
  ...
  （v32 / v36 起不再有「从网站同步课程 / 从网站导入教师与场地名」那几个入口 —— 机构口径是"现在都以后端为主"：
  内容文件只在**新装系统**时作为初始数据进库，之后课程、教师、教室都在后台维护。）
...
- 后台改价不会自动出现在宣传页，需要「`npm run site:export`（把库里的网站内容写回 `data/site/*.md`）→ 提交发布」才会上线
  （内容文件的真源归属见 [docs/后端开发方案.md](./docs/后端开发方案.md) §10.1）；
- 线上那份后台连不上后端（静态站），因此**只能在本机使用**：页面会明确提示这一点；
- ~~网站内容进库做只读镜像~~ ✅ **已改为更好的做法**（v15 起，v19 起口径定稿）：
  教师 / 课程卡片 / 课程分区 / 课程正文 / 报价 / **学生案例**都在库里。
  **构站时连得上后端就用库里的；连不上那五块就是空白**（机构确认：需要后端数据的地方，
  没连上后端就该是空的）—— 没有"一半来自库、一半来自文件"的中间态。
  其余页面（首页 / 关于 / 联系 / FAQ / 课表 / 特色课程 / 品牌）**也已经在库里**（后台「网站内容」页可改，v19–v22 起）；
  `data/site/*.md` 是它们的**导出产物**（`npm run site:export` 从库写回），只在没连后端 / 模板模式（线上 Pages）时当兜底，
  手工改会在下次导出时被覆盖。想本地看模板那一份：`SITE_CONTENT_SOURCE=template`（见技术架构 §5.1）。
...
| `npm run sync-content` | `data/site/*.md` → `data/site/*.ts`（由 `npm run site:export` 自动调用） |
| `npm run site:export` | 把后台（SQLite）里的网站内容导出回 `data/site/*.md`（六个文件），并接着跑 `sync-content`；导出后这六个文件**以后台为准**，手工改动会被下次导出覆盖 |
| `npm run site:export -- --check` | 只比对不写盘：六个文件与后台不一致时**退出码 1**（推送前确认"网站与后台同步了"） |
...
渲染为 HTML，因此线上要看到内容改动就得重新构建并部署（后台改完先 `npm run site:export`
把库写回 `data/site/*.md` 并提交；本地 dev 下跑一次 `npm run sync-content` 即生效）。
...
└── admin/                教务后台路由组
    ...
    └── (dashboard)/      后台页面（整棵子树被 RequireAuth 包住）
        ...
        └── inquiries/ followups/ finance/ pricing/   咨询 / 待跟进 / 收费 / 报价
            courses/ content/ stats/ scripts/ data/    课程库 / 网站内容 / 统计 / 话术 / 数据与备份
...
├── backend/              **服务层**：`api.ts`（当前 109 个方法，以 `contract.ts` 为准）+ 领域纯函数（排课与冲突、课时账本、报价、统计…）
...
| [docs/后台API约定.md](./docs/后台API约定.md) | 接后端的人 | 当前 105 个接口的分组、服务端必须复核的校验、迁移步骤 |
