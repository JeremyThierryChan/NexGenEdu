...
按「将来会有服务端」的形状实现：页面只调 `lib/backend/api.ts`，数据暂存在浏览器 localStorage（**伪后端阶段**）。

- 今日概览、咨询（排课可行性）、学生档案（含信息采集表）、教师、教室、课程安排、日历、课表与占用、统计、待跟进、收费、报价、数据与备份；
- 课时与金额都走账本：报课/续费/退课、收款/退款、请假扣课时、补课、撤销，每次改动留操作日志；
- 报价与教师分成是**可配置数据**，后台可直接给家长试算；
- 登录是**演示用的假登录**（`admin` / `689992`）：只是门，不是锁。

### 明确还没做（不是遗漏，是范围外）

真实服务端与数据库、真实鉴权、支付、短信/微信通知、家长与学生账号、RBAC、多校区 —— 详见 [路线图](#路线图) 与 [PROJECT.md](./PROJECT.md)。

### 已知边界

后台数据只存在**这台电脑的浏览器**里（换设备/清缓存会丢，靠「数据与备份」页导出 JSON 搬运）；
...
| Phase 4 | 教务后台：概览、学生、教师、教室、课程、日历 | ✅ 已完成（伪后端版本） |
| Phase 5 | 核心业务：排课、冲突检测、课时扣减、收费与退费、请假补课、统计、搜索与日志 | ✅ 已完成（伪后端版本） |
...
| 下一步 | 接真实服务端与数据库（只替换 `lib/backend/api.ts`，页面不改） | 计划中 |
...
| [docs/后台API约定.md](./docs/后台API约定.md) | 接后端的人 | 97 个接口的分组、服务端必须复核的校验、迁移步骤 |
